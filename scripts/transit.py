...
FREEFRLOWNODES_dbf = os.path.join(
    WORKING_FOLDER, config["transit"]["FREEFLOW_nodes_DBF"]
)
...
CALTRAIN_STOPS_DOWNTOWN = [14654, 14655]  # Transbay Terminal  # 4th and King
CALTRAIN_STOPS_SF = [
    14656,  # 22nd Street
...
class SanFranciscoNodeChecker:
    ...
    def inSF(self, nodenum):
        ...
        if (
            coords[0] > 6019000
            and coords[0] < 6024550
            and coords[1] > 2086000
            and coords[1] < 2140000
        ):
            return True
...
def getTransitVolumes(system, df):
    ...
    if system == TRN_BART_TUBE:
        ...
        prefixes = ["100_"]
        sf = BART_STOPS_SF
        nonsf = [16510, 16534]  # W Oakland  # Lake Merrit
    ...
    if len(sf) == 0:
        outbound = tmp[
            tmp["A"].map(lambda x: sfNC.inSF(x)) & ~tmp["B"].map(lambda x: sfNC.inSF(x))
        ]["AB_VOL"].sum()
        inbound = tmp[
            tmp["B"].map(lambda x: sfNC.inSF(x)) & ~tmp["A"].map(lambda x: sfNC.inSF(x))
        ]["AB_VOL"].sum()
        return [inbound, outbound]
    else:
        outbound = tmp[
            tmp["A"].map(lambda x: x in sf) & tmp["B"].map(lambda x: x in nonsf)
        ]["AB_VOL"].sum()
        inbound = tmp[
            tmp["B"].map(lambda x: x in sf) & tmp["A"].map(lambda x: x in nonsf)
        ]["AB_VOL"].sum()
        return [inbound, outbound]
...
def getTransitBoardings(df, timePeriod):
    ...
    df_bus_sliced2 = tmp[
        tmp["NAME"].str.slice(3, 4).str.isnumeric()
        & ~tmp["NAME"].str.slice(3, 5).isin(["59", "60", "61"])
    ]
    transit[TRN_MUNI_BUS] += df_bus_sliced2["AB_BRDA"].sum()
    df_rail_sliced2 = tmp[
        ~tmp.index.isin(df_bus_sliced1.index)
        & ~tmp.index.isin(df_rail_sliced1.index)
        & ~tmp.index.isin(df_bus_sliced2.index)
    ]
    transit[TRN_MUNI_RAIL] += df_rail_sliced2["AB_BRDA"].sum()
    transit[TRN_MUNI_TOTAL][0] = transit[TRN_MUNI_RAIL][0] + transit[TRN_MUNI_BUS][0]

    transit[TRN_MUNI_GEARY] = [getTransitBoardingsForLine("MUN38", df)]
    transit[TRN_MUNI_VN] = [
    ...
    transit[TRN_BART_TUBE] = getTransitVolumes(TRN_BART_TUBE, df)
    transit[TRN_BART_SM] = getTransitVolumes(TRN_BART_SM, df)
    transit[TRN_CALTRAIN_SM] = getTransitVolumes(TRN_CALTRAIN_SM, df)
    transit[TRN_ACTRANSIT] = getTransitVolumes(TRN_ACTRANSIT, df)
    transit[TRN_GGT] = getTransitVolumes(TRN_GGT, df)
    transit[TRN_SAMTRANS] = getTransitVolumes(TRN_SAMTRANS, df)
    ...
    if missing_dbf_files:
        ...
        for dbf_file in dbf_files:
            csv_file = os.path.join(
                WORKING_FOLDER, os.path.splitext(os.path.basename(dbf_file))[0] + ".csv"
            )
            dbf = Dbf5(dbf_file)
...
df_muni_ridership = pd.DataFrame(muni_rows[1:], columns=muni_rows[0]).set_index(
    muni_rows[0][0]
)
df_muni_ridership["San Francisco"] = df_muni_ridership["San Francisco"].apply(
    lambda x: f"{x:,}"
)
df_muni_ridership.loc[TRN_XFER_RATE] = f"{round(rate,2)}"
df_rail_boardings = pd.DataFrame(
    rail_boarding_rows[1:], columns=rail_boarding_rows[0]
).set_index(rail_boarding_rows[0][0])
df_rail_volume = pd.DataFrame(rail_vol_rows[1:], columns=rail_vol_rows[0]).set_index(
    rail_vol_rows[0][0]
)
df_bus_volume = pd.DataFrame(bus_vol_rows[1:], columns=bus_vol_rows[0]).set_index(
    bus_vol_rows[0][0]
)
...
class SimwrapperMapConstructor:
    ...
    def build_maps(
        ...
        df["geometry"] = pd.Series(
            [LineString([x, y]) for x, y in zip(df["A_xy"], df["B_xy"])]
        )
    ...
    def fix_caltrain_shapes(self):
        ...
        ct = df.loc[
            df["MODE"].eq(26)
            & df["A"].isin(self.caltrain_nodes)
            & df["B"].isin(self.caltrain_nodes)
        ]
        new_rows_all = []
        for idx, row in ct.iterrows():
            new_rows = self.split_link(row, self.caltrain_nodes)
            new_rows.index = (
                pd.RangeIndex(start=idx * 10, stop=idx * 10 + len(new_rows), step=1)
                / 10
            )
    ...
    def split_link(self, row, nodes):
        ...
        if abs(iA - iB) == 1:
            return pd.DataFrame(
                data=[[row["A"], row["B"], row["MODE"], row["NAME"], row["AB_VOL"]]],
...
class MapDataConstructor:
    ...
    def create_data(self):
        ...
        self.am_df = self.am_df.groupby("AB", as_index=False).agg(
            {"AB_VOL": "sum", "crowd": "max"}
        )
        self.pm_df = self.pm_df.groupby("AB", as_index=False).agg(
            {"AB_VOL": "sum", "crowd": "max"}
        )
    ...
    def get_crowd(self, df, tp):
        agg_df = (
            df.groupby("AB")
            .agg(AB_VOL_sum=("AB_VOL", "sum"), PERIODCAP_sum=("PERIODCAP", "sum"))
            .reset_index()
        )
        agg_df["crowd"] = (agg_df["AB_VOL_sum"] * self.peakHour_factor[tp]) / (
            agg_df["PERIODCAP_sum"] / self.tp_duration[tp]
        )
    ...
    def split_link(self, row, nodes):
        ...
        if abs(iA - iB) == 1:
            return pd.DataFrame(
                data=pd.DataFrame(
                    data=[[row["A"], row["B"], row["MODE"], row["AB_VOL"]]],
                    columns=["A", "B", "MODE", "AB_VOL"],
                )
            )
...
mapConstructor = SimwrapperMapConstructor(
