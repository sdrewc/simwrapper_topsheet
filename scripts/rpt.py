...
rp_person = os.path.join(
    WORKING_FOLDER, "daysim", "abm_output1", config["resident_purpose"]["RP_PERSON"]
)
rp_tour = os.path.join(
    WORKING_FOLDER, "daysim", "abm_output1", config["resident_purpose"]["RP_TOUR"]
)
rp_trips = os.path.join(
    WORKING_FOLDER,
    "daysim",
    "abm_output1",
    config["resident_purpose"]["RP_DISAG_TRIPS"],
)
...
class ResidentPurposes:
    ...
    def __init__(self, eqvfile):
        (self.distToName, self.distToTaz, self.tazToDist, _numdists) = readEqvFile(
            eqvfile
        )
        self.purposesByTimeperiod = False
        self.trip_file = os.path.join(
            WORKING_FOLDER, "daysim", "abm_output1", "trips_temp.h5"
        )
        self.tour_file = os.path.join(
            WORKING_FOLDER, "daysim", "abm_output1", "tours_temp.h5"
        )
        tp_dict = {
            "Daily": None,
            "AM": None,
    ...
    def getChampPurpose(self, row, tour=False):
        ...
        return purpose_map.get(
            xfer_purp, ResidentPurposes.RP_ROWS[4]
        )  # Other as default
    ...
    def create_trip(self):
        ...
        trip_df = pd.read_csv(
            ResidentPurposes.RP_DISAG_TRIPS,
            sep="\t",
            usecols=[
                "hhno",
                "pno",
        ...
        tour_df = pd.read_csv(
            ResidentPurposes.RP_TOUR, sep="\t", usecols=["id", "parent"]
        )
        hh_df = pd.read_csv(ResidentPurposes.RP_HH, sep="\t", usecols=["hhno", "hhtaz"])
        per_df = pd.read_csv(
            ResidentPurposes.RP_PERSON, sep="\t", usecols=["hhno", "pno", "pptyp"]
        )
        ...
        trip_df["origin_district"] = trip_df["otaz"].map(
            lambda x: self.tazToDist[x][0] if x in self.tazToDist else None
        )
        trip_df["destination_district"] = trip_df["dtaz"].map(
            lambda x: self.tazToDist[x][0] if x in self.tazToDist else None
        )
        trip_df["res_district"] = trip_df["hhtaz"].map(
            lambda x: self.tazToDist[x][0] if x in self.tazToDist else None
        )
    ...
    def getResidentPurposes(self, mode, timePeriod, sourceTaz="hhtaz"):
        ...
        *timePeriod* can be one of "Daily", "AM" or "PM"
        ...
        dist_map = {
            "hhtaz": "res_district",
            "otaz": "origin_district",
            "dtaz": "destination_district",
        }
        columns_to_load = [
            "TimePeriod",
            "trip_purpose",
            dist_map[sourceTaz] if mode == "district" else sourceTaz,
        ]
        pivot_df = None
        with pd.HDFStore(self.trip_file, "r") as store:
            ...
            iterator = store.select(
                "root", columns=columns_to_load, chunksize=chunksize
            )
            ...
            for df in iterator:
                ...
                temp_pivot = grouped.pivot(
                    index="trip_purpose",
                    columns=dist_map[sourceTaz] if mode == "district" else sourceTaz,
                    values="counts",
                ).T
    ...
    def create_tour(self):
        ...
        tour_df = pd.read_csv(
            ResidentPurposes.RP_TOUR,
            sep="\t",
            usecols=["id", "hhno", "pno", "parent", "pdpurp", "tlvorig"],
        )
        hh_df = pd.read_csv(ResidentPurposes.RP_HH, sep="\t", usecols=["hhno", "hhtaz"])
        per_df = pd.read_csv(
            ResidentPurposes.RP_PERSON, sep="\t", usecols=["hhno", "pno", "pptyp"]
        ...
        tour_df["TimePeriod"] = tour_df.apply(
            lambda row: self.getTimePeriod(row, True), axis=1
        )
        tour_df["purpose"] = tour_df.apply(
            lambda row: self.getChampPurpose(row, True), axis=1
        )
        tour_df["district"] = tour_df["hhtaz"].map(
            lambda x: self.tazToDist[x][0] if x in self.tazToDist else None
        )

        tour_df = tour_df[["TimePeriod", "purpose", "district", "hhtaz"]]
...
sum_df = pd.DataFrame(
    [sum_all_rows, sum_first_12_rows], index=["Bay Area", "San Francisco"]
)
